...
# This dedicated sibling probe shares the fixed root's text prefix but is
# outside it. No removal is performed here: all three files remain as evidence.
set(outside_probe "${CONTRACT_ROOT}-outside-probe/owned-${other_nonce}")
file(MAKE_DIRECTORY "${outside_probe}/nested")
file(WRITE "${outside_probe}/.pure-midi-owner" "${nonce}")
file(WRITE "${outside_probe}/keep.txt" "outside fixed root guard\n")
file(WRITE "${outside_probe}/nested/keep.txt" "outside fixed root nested guard\n")
foreach(file IN ITEMS .pure-midi-owner keep.txt nested/keep.txt)
  file(SHA256 "${outside_probe}/${file}" before_hash)
  string(MAKE_C_IDENTIFIER "${file}" key)
  set("outside_hash_${key}" "${before_hash}")
endforeach()
reject(outside-fixed-root "${outside_probe}" "${nonce}" "${outside_probe}/keep.txt")
foreach(file IN ITEMS .pure-midi-owner keep.txt nested/keep.txt)
  if(NOT EXISTS "${outside_probe}/${file}")
    message(FATAL_ERROR "outside-fixed-root cleanup removed ${file}")
  endif()
  file(SHA256 "${outside_probe}/${file}" after_hash)
  string(MAKE_C_IDENTIFIER "${file}" key)
  if(NOT after_hash STREQUAL "${outside_hash_${key}}")
    message(FATAL_ERROR "outside-fixed-root cleanup changed ${file}")
  endif()
endforeach()
message(STATUS "PASS outside fixed root retained byte-for-byte: ${outside_probe}")
...
message(STATUS "PASS cleanup contract: 13 ownership/reparse cases")
