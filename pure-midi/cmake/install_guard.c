...
static int hold(const wchar_t *path, int directory) {
  ...
  if (!GetFileInformationByHandle(h, &info) ||
    ...
    CloseHandle(h); return error("reparse, hardlink, or wrong endpoint type");
  }
...
#ifdef PURE_MIDI_INSTALL_GUARD_FIXTURE
static void fixture_gate(const wchar_t *phase) {
  static int reached;
  wchar_t ready[128], release[128], selected[32]=L"queued";
  GetEnvironmentVariableW(L"MIDI_GUARD_TEST_PHASE",selected,32);
  if (wcscmp(selected,phase)) return;
  if (reached++ || !GetEnvironmentVariableW(L"MIDI_GUARD_TEST_READY",ready,128) ||
      !GetEnvironmentVariableW(L"MIDI_GUARD_TEST_RELEASE",release,128)) return;
...
#ifdef PURE_MIDI_INSTALL_GUARD_FIXTURE
    wchar_t failure[16];
    if (GetEnvironmentVariableW(L"MIDI_GUARD_TEST_FAIL_AFTER_WRITE",failure,16) &&
...
static int client(int argc,wchar_t **argv) {
  ...
  if (!r->op || !GetEnvironmentVariableW(L"PURE_MIDI_INSTALL_CHANNEL",pipe_name,128) ||
      wcsncmp(pipe_name,L"\\\\.\\pipe\\pure-midi-install-",wcslen(L"\\\\.\\pipe\\pure-midi-install-"))) {
  ...
  if (ok) {
    ...
    ok=directory!=INVALID_HANDLE_VALUE && GetFileInformationByHandle(directory,&identity) &&
      ...
      GetEnvironmentVariableW(L"PURE_MIDI_INSTALL_CAPABILITY",r->capability,65)==64 &&
...
int wmain(int argc,wchar_t **argv) {
  ...
  if (argc==3 && !wcscmp(argv[1],L"--test-global-owner")) {
    ...
    CloseHandle(h);
    wchar_t name[128];
    swprintf(name,128,L"Global\\pure-midi-stage-%08lx-%08lx%08lx",
  ...
  swprintf(mutex_name,128,L"Global\\pure-midi-stage-%08lx-%08lx%08lx",
    info.dwVolumeSerialNumber,info.nFileIndexHigh,info.nFileIndexLow);
  ...
  wcscpy(pipe_name,L"\\\\.\\pipe\\pure-midi-install-"); size_t offset=wcslen(pipe_name);
  for (int i=0;i<16;++i) swprintf(pipe_name+offset+i*2,3,L"%02x",nonce[i]);
  if (!SetEnvironmentVariableW(L"PURE_MIDI_INSTALL_CHANNEL",pipe_name)) return 1;
  BYTE secret[32];
  ...
  if (!SetEnvironmentVariableW(L"PURE_MIDI_INSTALL_CAPABILITY",capability)) return 1;
  wchar_t *command=calloc(32768,sizeof(wchar_t)), option[PATH_CAP+64];
  if (!command || !argument(command,argv[4])) return 1;
  const wchar_t *names[]={L"STAGE_PREFIX",L"MIDI_INSTALL_CONTEXT",L"MIDI_INSTALL_MODE",L"MIDI_INSTALL_COMPONENT"};
