...
try {
    ...
    $pluginUnresolvedPreThrowPatterns = @(
        ...
        '(?s)if \(\$apiMappings\.ContainsKey\(\$key\) -and \$apiMappings\[\$key\]\.Path -ne \$resolved\) \{\s*if \(\$pe\.Group -ceq ''pure-gnuplot-platform-plugin''\) \{ \$gnuplotPlatformPluginUnresolvedEdgeCount\+\+ \}\s*throw "API-set contract mapped inconsistently'
    )
