...
set(authority "")
foreach(name expected_files expected_dirs readme_template archive_hash extracted work
  ...
  string(APPEND authority "set(${name} [==[${${name}}]==])\n")
endforeach()
foreach(path IN LISTS expected_files)
  string(APPEND authority "set(source_hash_${path} ${source_hash_${path}})\n")
endforeach()
file(WRITE "${work}/archive-authority.cmake" "${authority}")
gl_audit_run(isolated-extracted "${PURE_GL_WINDOWS_SYSTEM_DIRECTORY}/WindowsPowerShell/v1.0/powershell.exe"
  -NoProfile -NonInteractive -File "${extracted}/tests/IsolateSource.ps1"
  -SourceRoot "${SOURCE_DIR}" -BuildRoot "${BINARY_DIR}" -CMake "${CMAKE_COMMAND}"
  -Bootstrap "${extracted}/tests/VerifyExtractedSource.cmake" -Authority "${work}/archive-authority.cmake")
