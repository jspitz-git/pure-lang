...
function(snapshot root output)
  file(GLOB_RECURSE paths LIST_DIRECTORIES FALSE RELATIVE "${root}" "${root}/*")
  set(rows)
  foreach(path IN LISTS paths)
    if(IS_DIRECTORY "${root}/${path}" OR IS_SYMLINK "${root}/${path}")
      file(REAL_PATH "${root}/${path}" destination)
      list(APPEND rows "${path}|link-or-directory|${destination}")
    else()
...
execute_process(COMMAND ${make} clean DLL=.dll WORKING_DIRECTORY "${copy}"
  COMMAND_ERROR_IS_FATAL ANY)
foreach(name GL GL_ARB GL_EXT GL_NV GL_ATI GLU GLUT)
  if(EXISTS "${copy}/${name}.o" OR NOT EXISTS "${copy}/${name}.c" OR NOT EXISTS "${copy}/${name}.pure")
    message(FATAL_ERROR "Clean failed exact object/wrapper ownership for ${name}")
  endif()
endforeach()
foreach(name unrelated.dll unrelated.o pure-gl.dll.note)
  if(NOT EXISTS "${copy}/${name}")
    message(FATAL_ERROR "Clean deleted unrelated file: ${name}")
  endif()
endforeach()
foreach(name GL GL_ARB GL_EXT GL_NV GL_ATI GLU GLUT)
  ...
  endforeach()
endforeach()
execute_process(COMMAND ${make} --jobs=4 generate DLL=.dll "CC=${C_COMPILER}"
...
endforeach()
message(STATUS "PURE_GL_CLEANUP_CONTRACT_OK legacy_negative=35 inherited_negative=15 protected_writes=0 clean=1 realclean=1 generate=1 workers=4")
