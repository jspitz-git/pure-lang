...
function(ci_validate_trigger trigger)
  ...
  foreach(required IN LISTS expected)
    set(required_count 0)
    foreach(candidate IN LISTS actual)
      if(candidate STREQUAL required)
        math(EXPR required_count "${required_count} + 1")
      endif()
    endforeach()
    if(NOT required_count EQUAL 1)
      message(FATAL_ERROR
        "CI_CONTRACT_TRIGGER: ${trigger}.paths must contain exactly one "
        "required Bonjour path [${required}]; actual=[${actual}]")
    endif()
  endforeach()
endfunction()
...
ci_count_text("${pure_run}" "${runtime_stager_block}" runtime_stager_count)
if(NOT runtime_stager_count EQUAL 1)
  message(FATAL_ERROR
...
if(NOT MUTATION_MODE)
  ...
  function(ci_mutate_named_step_line subject title old replacement output)
    set(job_marker "  windows-pure-bonjour:\n")
    string(FIND "${subject}" "${job_marker}" job_start)
    if(job_start EQUAL -1)
      message(FATAL_ERROR
        "CI_CONTRACT_FIXTURE: cannot find Windows PureBonjour job")
    endif()
    string(FIND "${subject}" "\n  macos-15-arm64:" job_end)
    if(job_end EQUAL -1 OR NOT job_start LESS job_end)
      string(LENGTH "${subject}" job_end)
    endif()
    set(marker "      - name: ${title}\n")
    string(SUBSTRING "${subject}" ${job_start} -1 job_and_after)
    string(FIND "${job_and_after}" "${marker}" step_relative)
    if(step_relative EQUAL -1)
      message(FATAL_ERROR
        "CI_CONTRACT_FIXTURE: cannot find step ${title}")
    endif()
    math(EXPR step_start "${job_start} + ${step_relative}")
    if(NOT step_start LESS job_end)
      message(FATAL_ERROR
        "CI_CONTRACT_FIXTURE: step ${title} is outside Windows PureBonjour")
    endif()
    ...
    if(next_relative EQUAL -1)
      set(step_end ${job_end})
    else()
      math(EXPR step_end "${after_marker} + ${next_relative} + 1")
      if(step_end GREATER job_end)
        set(step_end ${job_end})
      endif()
    endif()
  ...
  string(REPLACE "      - \"pure-bonjour/**\""
    "      - \"pure-bonjour/**\"\n      - \"pure-bonjour/**\""
    mutated "${workflow}")
  ci_expect_rejected(duplicate-bonjour-trigger-path "${mutated}" TRIGGER)
  ...
  ci_mutate_named_step_line("${mutated}"
    ...
    "          }\n          if ($LASTEXITCODE -ne 0) { throw \"Pure runtime DLL staging failed\" }"
    mutated)
  ci_expect_rejected(unreachable-sdk-runtime-stager "${mutated}" PURE)
