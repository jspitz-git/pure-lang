...
function(require_no_forbidden_file path)
    ...
    foreach(spelling IN ITEMS "${normalized}" "${native}")
      ...
      while(hex_offset LESS needle_hex_length)
        ...
        if(letter_index GREATER_EQUAL 0)
          list(GET ascii_lower_hex ${letter_index} byte)
        endif()
  ...
  while(offset LESS file_size)
    ...
    list(LENGTH ascii_upper_hex ascii_letter_count)
    math(EXPR ascii_letter_last "${ascii_letter_count} - 1")
    foreach(letter_index RANGE 0 ${ascii_letter_last})
      list(GET ascii_upper_hex ${letter_index} upper_byte)
      list(GET ascii_lower_hex ${letter_index} lower_byte)
      string(REPLACE "${upper_byte}:" "${lower_byte}:" chunk "${chunk}")
    endforeach()
