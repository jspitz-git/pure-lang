...
#ifdef __MINGW32__
	  /* appending / to denote a directory (if pattern == "") doesn't
	     work with Windows stat(); we need to check the result of stat()
	     instead */
	  int slash = patlen > 0;
	  char *fullname = (char *) __alloca (dirlen + slash + patlen + 1);
	  if (slash) {
