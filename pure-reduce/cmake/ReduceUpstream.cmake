...
function(_pure_reduce_runtime_directory_is_complete DIRECTORY MANIFEST OUT_VALID)
  set(_valid TRUE)
  if(NOT IS_DIRECTORY "${DIRECTORY}" OR
     NOT EXISTS "${MANIFEST}" OR IS_DIRECTORY "${MANIFEST}")
    set(_valid FALSE)
  endif()

  set(_expected_relatives)
  if(_valid)
  ...
  if(_valid)
    foreach(_line IN LISTS _manifest_lines)
      ...
      if(NOT _sha256_length EQUAL 64 OR IS_ABSOLUTE "${_relative}" OR
      ...
      cmake_path(IS_PREFIX DIRECTORY "${_file}" NORMALIZE _inside_directory)
      if(NOT _inside_directory OR NOT EXISTS "${_file}" OR
         IS_DIRECTORY "${_file}")
        set(_valid FALSE)
        break()
      endif()
      ...
      list(APPEND _expected_relatives "${_relative}")
    endforeach()
  endif()

  if(_valid)
    file(GLOB_RECURSE _actual_files LIST_DIRECTORIES FALSE "${DIRECTORY}/*")
    set(_actual_relatives)
    foreach(_file IN LISTS _actual_files)
      cmake_path(RELATIVE_PATH _file BASE_DIRECTORY "${DIRECTORY}"
        OUTPUT_VARIABLE _relative)
      list(APPEND _actual_relatives "${_relative}")
    endforeach()
    list(SORT _expected_relatives)
    list(SORT _actual_relatives)
    if(NOT _actual_relatives STREQUAL _expected_relatives)
      set(_valid FALSE)
    endif()
  endif()
